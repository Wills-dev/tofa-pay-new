...
export const useGetAllScheduled = () => {
  ...
  const getSchedule = async () => {
    try {
      const data = await axiosInstance.get(`/get-all-schedule`);
      if (user.role === "Partner" || user.role === "Partner Agents") {
        const filterPartnerList = data?.data?.schedule?.rows?.filter(
          (list: ScheduleData) => list.companyInitial === user?.companyInitial
        );
        setGetAllSchedule(filterPartnerList);
      } else {
        setGetAllSchedule(data?.data?.schedule?.rows);
      }
      setLoading(false);
    } catch (error: any) {
      setLoading(false);
