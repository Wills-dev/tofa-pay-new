...
import { useGetAllScheduledListTableDetails } from "@/hooks/useGetAllScheduledListTableDetails";
...
const ScheduledListTableDetails = () => {
  const { tableId } = useParams();

  const { exportTableToExcel, excelLoading } = useExportSchedule();
  ...
  const {
    ...
    handleResetFilter,
    paginate,
  } = useGetAllScheduledListTableDetails();

  const handleExport = () => {
    const filename = "Scheduled file";
    exportTableToExcel(getAllScheduleListTableDetails, filename, tableId);
  };
    ...
    <div className={` bg-main-bg min-h-screen  flex-1 w-full`}>
      ...
      <div className={`${activeMenu ? ` md:ml-72 ` : `flex-2`}`}>
        ...
        <div className="m-2 md:m-10 p-2 md:p-10 bg-white rounded-3xl pt-20 max-sm:mt-0">
          <Header category="Schedule" title="Scheduled list table details" />
          {loading ? (
            <ListLoader />
          ) : (
            <>
              <div className="flex justify-end mb-3">
                {excelLoading ? (
                  <button className="bg-disabled flex items-center justify-center gap-1 text-color-dark-font w-44 whitespace-nowrap h-11 rounded-lg font-semibold cursor-pointer">
                    <Spin />
                  </button>
                ) : (
                  <button
                    ...
                    className="bg-primary-color hover:bg-secondary-color flex items-center justify-center gap-1 text-color-dark-font w-44 whitespace-nowrap h-11 rounded-lg font-semibold cursor-pointer"
                  >
                ...
                <div className="flex items-center gap-4 flex-wrap">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      {Object.keys(
                        (getAllScheduleListTableDetails &&
                          getAllScheduleListTableDetails[0]) ??
                          {}
                      ).map((key) => (
                  ...
                  {selectedKey && (
                    <div className="flex items-center gap-1">
                      ...
                      >
                        ...
                        {Array.from(
                          new Set(
                            getAllScheduleListTableDetails?.map(
                      ...
                      <button
                        ...
                        onClick={() => handleResetFilter(tableId)}
                      >
                        Reset
                      </button>
                    </div>
...
export default ProtectedRoutes(ScheduledListTableDetails, [
  "TOFA Pay Admin",
  "Finance",
  "Super Admin",
]);
